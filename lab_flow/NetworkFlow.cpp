...
{
  vector<Vertex> startVertex = g_.getVertices();
  vector<Edge> startEdge = g_.getEdges();

  for(unsigned int i = 0; i < startVertex.size(); i++)
  {
    residual_.insertVertex(startVertex[i]);
    flow_.insertVertex(startVertex[i]);
  }
  for(unsigned int i = 0; i < startEdge.size(); i++)
  {
    residual_.insertEdge(startEdge[i].source, startEdge[i].dest);
    residual_.insertEdge(startEdge[i].dest, startEdge[i].source);

    residual_.setEdgeWeight(startEdge[i].source, startEdge[i].dest, startEdge[i].getWeight());
    residual_.setEdgeWeight(startEdge[i].dest, startEdge[i].source, 0);

    flow_.insertEdge(startEdge[i].source, startEdge[i].dest);
    flow_.setEdgeWeight(startEdge[i].source, startEdge[i].dest, 0);
  }
  // YOUR CODE HERE
}
