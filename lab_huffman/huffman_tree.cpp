...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    {
      TreeNode * pusher = new TreeNode(frequencies[i]);
      singleQueue.push(pusher);
    }
    ...
    while(singleQueue.size() + mergeQueue.size() != 1)
    {
      ...
      TreeNode * levelUp = new TreeNode(tmp1->freq.getFrequency() + tmp2->freq.getFrequency());

      levelUp->left = tmp1;
      levelUp->right = tmp2;
      mergeQueue.push(levelUp);
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    TreeNode * reader = NULL;

    if(bfile.getNextBit())
    {
      reader = new TreeNode(Frequency(bfile.getNextByte(), 0));
    }
    else
    {
      reader = new TreeNode(0);
      reader->left = readTree(bfile);
      reader->right = readTree(bfile);
    }
